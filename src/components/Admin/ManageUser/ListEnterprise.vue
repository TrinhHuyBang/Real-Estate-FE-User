<template>
  ...
  >
    ...
    <el-table-column align="center" label="Giấy đăng ký kinh doanh" width="200">
      <template slot-scope="scope">
        <el-image
          :lazy="true"
    ...
    <el-table-column align="center" label="Trạng thái" width="100">
      <template slot-scope="scope">
        <el-tag v-if="scope.row.account_status" type="success"> Hoạt động </el-tag>
...
export default {
  ...
  methods: {
    ...
    openDeleteConfirm(enterprise) {
      this.$confirm(
        "Bạn muốn khoá tài khoản của " + enterprise.name + ". Bạn muốn tiếp tục?", "Xác nhận",
        {
    ...
    openRejectConfirm(enterprise) {
      this.$confirm(
        "Bạn muốn từ chối yêu cầu đăng ký của " + enterprise.name + ". Bạn muốn tiếp tục?", "Xác nhận",
        {
