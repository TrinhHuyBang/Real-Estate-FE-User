<template>
  <div>
    ...
    <div>
        ...
        <div class="row user-logo-name">
          <div class="col-4 user-logo">
            <img
              v-if="broker && broker.info && broker.info.avatar"
              :src="broker.info.avatar"
              alt="avatar"
            />
          ...
          <div class="col-6 user-name-post">
            <h2 class="user-name">
              {{
                broker && broker.info && broker.info.name
                  ? broker.info.name
                  : ""
              }}
            </h2>
            <div>
              <div class="number-post">
                ...
                <h2>{{ broker.count_posts }}</h2>
              </div>
              <div class="number-post">
                <h6>Tổng số lần tư vấn</h6>
                <h2>{{ broker.number_consultations }}</h2>
              </div>
              <div class="number-post">
                <h6>Đánh giá trung bình</h6>
                <h2 v-if="broker.rating">{{ broker.rating }} <i style="color: #ff9900;" class="fa-solid fa-star"></i></h2>
                <h2 v-else>N/A</h2>
              </div>
            </div>
          </div>
    ...
    <div class="container">
      <div class="row">
        <div class="col-8">
          ...
          <div>
            <h6 class="title">Loại bất động sản và khu vực môi giới</h6>
            <ul>
          ...
          <div v-if="broker && broker.description">
            <h6 class="title">Lời tự giới thiệu của nhà môi giới</h6>
            <span>
              {{ broker.description }}
            </span>
          </div>
          <list-reviews v-if="broker.reviews?.length" :reviews="broker.reviews"/>
        </div>
        <div class="col-4">
          <div class="project-contact">
              <h6>Thông tin liên hệ</h6>
              <p>Để biết thêm các thông tin chi tiết, vui lòng liên hệ với doanh nghiệp qua các phương thức liên hệ dưới đây</p>
              <el-button @click="coppyClipboard(broker.info?.phone)" type="primary">
              ...
              <el-button @click="activeContactModal = !activeContactModal" type="info">
    ...
    <user-list :user_id="broker.user_id" type="sell" />
    <user-list :user_id="broker.user_id" type="rent" />
    </div>
    <ContactModal :isActive="activeContactModal" :email="broker?.info?.email" :type="'broker'" @closeContactModal="activeContactModal = !activeContactModal" />
  </div>
</template>
...
import ListReviews from '@/components/PhoneBook/Broker/ListReviews.vue'
...
.project-contact .el-button+.el-button {
  margin-left: 0px;
}
</style>